...
export default function StudySessionPage() {
  ...
  const { data: cardsData, isLoading: isCardsLoading, error: cardsError } = useCardsByDeck(id ?? "")
  ...
  const currentCard = sessionData.cards[currentCardIndex % Math.max(sessionData.cards.length, 1)]

  const handleRate = (rating: number) => {
    setIsRevealed(false)
    setCurrentCardIndex((prev) => (prev + 1) % Math.max(sessionData.cards.length, 1))
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-app-bg text-gray-200 relative overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative z-10">
        {currentCard ? (
          <StudyCard {...currentCard} isRevealed={isRevealed} onReveal={handleReveal} />
        ) : (
          <div className="text-center text-white">No cards available in this deck</div>
        )}
      </main>

      <StudyControls isRevealed={isRevealed} onReveal={handleReveal} onRate={handleRate} />
    </div>
  )
}
