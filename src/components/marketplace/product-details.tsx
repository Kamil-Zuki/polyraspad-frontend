    ...
    >
      <div className="bg-app-surface border border-app-border rounded-2xl shadow-xl max-w-2xl w-full max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-app-border">
          <h3 className="text-lg font-bold text-white">Try Smart Preview</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-4 overflow-y-auto flex-1">
          {isLoading && (
            <div className="flex items-center justify-center py-12 text-gray-400">
              <span className="text-sm">Loading sample cards...</span>
            </div>
          )}
          {isError && (
            <p className="text-gray-400 text-sm py-4">
              Could not load the demo. Try again later.
            </p>
          )}
          {!isLoading && !isError && items.length === 0 && (
            <p className="text-gray-400 text-sm py-4">No cards are available for preview.</p>
          )}
          {!isLoading && !isError && items.length > 0 && (
            <ul className="space-y-4">
              {items.map((card) => (
                ...
                >
                  <div className="text-xs text-gray-500 uppercase font-bold mb-2">Card</div>
                  <div className="text-white font-medium mb-2">{cardPreviewExpression(card)}</div>
                  <div className="text-gray-400 text-sm">
                    <span className="text-brand-primary">{cardPreviewWord(card)}</span>
                    {cardPreviewTranslation(card) ? ` — ${cardPreviewTranslation(card)}` : null}
                  </div>
                </li>
              ))}
            </ul>
    ...
    <div className="bg-app-surface p-5 rounded-xl border border-app-border group hover:border-white/10 transition-colors">
      ...
      {review.authorReply && (
        <div className="mt-3 pl-4 border-l-2 border-brand-primary/40 text-sm text-gray-500">
          <span className="text-gray-400">Author reply: </span>
          {review.authorReply}
        </div>
      )}
    </div>
