    ...
    <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border-app-border relative overflow-hidden group hover:border-brand-secondary/30 transition-all duration-300">
        ...
        <div className="flex justify-between items-start mb-4 relative z-10">
          <div>
            <div className="text-[10px] font-bold text-brand-secondary uppercase tracking-widest mb-1">{t("reviewsLabel")}</div>
        ...
        <div className="mt-4 text-xs text-gray-500 relative z-10">
          {!hasReviewGoal ? (
            ...
            t("cardsRemaining", { count: reviewsRemaining })
          ) : (
            t("goalCompleted")
          )}
        </div>
      ...
      <div className="glass-panel p-6 rounded-2xl border-app-border relative overflow-hidden group hover:border-brand-primary/30 transition-all duration-300">
        ...
        <div className="flex justify-between items-start mb-4 relative z-10">
          <div>
            <div className="text-[10px] font-bold text-brand-primary uppercase tracking-widest mb-1">{t("newWordsLabel")}</div>
            <div className="text-3xl font-bold text-white">
        ...
        <div className="mt-4 text-xs text-gray-500 relative z-10">
          {!hasNewGoal ? (
            ...
            t("newCardsRemaining", { count: newCardsRemaining })
          ) : (
            t("goalCompleted")
          )}
        </div>
...
function ContinueBanner({
  ...
  if (!currentProject) {
    return (
      ...
      >
        ...
        <h3 className="text-lg font-bold text-white mb-1">{t("noProject")}</h3>
        <p className="text-xs text-gray-400 mb-4 text-center">{t("noProjectDesc")}</p>
  ...
  if (!firstDeck) {
    return (
      ...
      >
        ...
        <h3 className="text-lg font-bold text-white mb-1">{t("noDecks")}</h3>
        <p className="text-xs text-gray-400 mb-4 text-center">{t("noDecksDesc")}</p>
        <span className="bg-white text-app-bg px-5 py-2.5 rounded-xl text-xs font-bold inline-block">
          {t("openDecks")}
        </span>
      </Link>
  ...
  return (
    <Link
      ...
      className="relative rounded-2xl overflow-hidden border border-app-border group cursor-pointer shadow-lg hover:shadow-brand-primary/10 transition-all duration-500 block min-h-[200px] bg-gradient-to-br from-brand-primary/20 via-brand-secondary/15 to-purple-900/30"
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,0.15),transparent_70%)] group-hover:scale-105 transition-transform duration-700 pointer-events-none" />
