...
export function DeckTree({ projectId }: DeckTreeProps) {
  ...
  const handleConfirmDeleteDeck = async () => {
    ...
    try {
      await deleteDeck.mutateAsync(deckToDeleteId)
      await refetch()
    } catch (err) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        {deckTree.map((deck) => (
          <DeckItem
            ...
            onDelete={(id) => setDeckToDeleteId(id)}
          />
        ))}
      </div>
      ...
      <ConfirmDialog
        isOpen={Boolean(deckToDeleteId)}
        onClose={() => setDeckToDeleteId(null)}
        onConfirm={handleConfirmDeleteDeck}
        title="Delete this deck?"
        description="All cards and sub-decks within this deck will be permanently deleted."
        variant="destructive"
