...
export function CardViewModal({ cardId, onClose, initialCard }: CardViewModalProps) {
  ...
  const fv = c?.note?.fieldValues
  const sentence = c ? noteFieldPlainString(fv, SENTENCE_MINING.Expression) : ""
  const targetWord = c ? noteFieldPlainString(fv, SENTENCE_MINING.Word) : ""
  const translation = c ? noteFieldPlainString(fv, SENTENCE_MINING.Translation) : ""
  const imgRaw = c ? noteFieldPlainString(fv, SENTENCE_MINING.Image) : ""
  const audRaw = c ? noteFieldPlainString(fv, SENTENCE_MINING.Audio) : ""
  const isUuid = (s: string) => /^[0-9a-f-]{36}$/i.test(s.trim())
  const imageUrlResolved = imgRaw && !isUuid(imgRaw) ? imgRaw : ""
  const imageIdResolved = imgRaw && isUuid(imgRaw) ? imgRaw : undefined
  const previewImageSrc = getPreviewImageSrc({
    imageId: imageIdResolved,
    imageUrl: imageUrlResolved || undefined,
    apiBaseUrl: API_BASE_URL,
  })
  const audioPlayUrl =
    audRaw && (audRaw.startsWith("http") || audRaw.startsWith("/") || audRaw.startsWith("blob:"))
      ? audRaw
      : ""
          ...
          {!hasContent ? (
            ...
            <div className="space-y-4">
              <p className="text-lg leading-relaxed text-white">
                &ldquo;{highlightTarget(sentence, targetWord)}&rdquo;
              </p>
              {previewImageSrc && (
                <div className="rounded-xl overflow-hidden border border-app-border bg-app-bg">
                  <PreviewImage
                    src={previewImageSrc}
                    fallbackSrc={imageUrlResolved || undefined}
                    alt="Card"
            ...
            <div className="space-y-4 animate-in fade-in duration-200 flex flex-col items-center text-center">
              {translation.trim() && (
                <p className="text-gray-300">&ldquo;{translation}&rdquo;</p>
              )}
              {targetWord.trim() && (
                <p className="text-sm">
              ...
              {audioPlayUrl && (
                <audio src={audioPlayUrl} controls className="w-full h-8 opacity-90 max-w-xs" />
              )}
            </div>
          )}
        </div>
            ...
            <div className="p-8 flex-1 overflow-y-auto flex flex-col min-h-0">
              {side === "FRONT" ? (
                ...
                <div className="space-y-6 animate-in fade-in duration-200 flex flex-col items-center justify-center text-center flex-1">
                  {translation.trim() && (
                    <p className="text-xl text-gray-300 max-w-2xl">&ldquo;{translation}&rdquo;</p>
                  )}
                  {targetWord.trim() && (
                  ...
                  {audioPlayUrl && (
                    <audio src={audioPlayUrl} controls className="w-full h-10 opacity-90 max-w-md" />
                  )}
                </div>
              )}
            </div>
