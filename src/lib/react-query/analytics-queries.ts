...
export function useSkillBalance(projectId: string) {
  return useQuery({
    queryKey: analyticsQueryKeys.skillBalance(projectId),
    queryFn: () => apiClient.analytics.getSkillBalance(projectId),
    enabled: !!projectId,
  });
}
