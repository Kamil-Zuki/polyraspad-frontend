...
export async function startImport(
  ...
  formData.append("deckId", config.deckId);

  const response = await fetch(
    `${API_BASE_URL}${API_ENDPOINTS.CARDS.IMPORT_FILE}`,
    {
      method: "POST",
      headers,
      body: formData,
...
export async function getImportJobStatus(jobId: string): Promise<ImportJobResponse> {
  ...
  const response = await fetch(
    `${API_BASE_URL}${API_ENDPOINTS.CARDS.IMPORT_JOB(jobId)}`,
    {
      method: "GET",
      headers,
    }
  ...
  if (!response.ok) {
    throw ApiError.fromResponse({ detail: `HTTP ${response.status}` }, response.status);
  }
